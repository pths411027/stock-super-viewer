...
function Content({
  ...
  return (
    <div className="mx-auto mt-2 flex w-full max-w-sm flex-col gap-2">
      <p className="text-primary text-sm font-bold">目標價格格</p>
      <div className="h-16 w-full overflow-hidden rounded-md border border-[rgba(212,175,89,0.12)] bg-[#1F1D16] p-2">
        <div className="flex h-full items-center justify-around">
          <div className="flex-1 pl-2">
            ...
            <div className="mr-1/2 text-cream text-lg font-bold">
              {data?.lastPrice}
            </div>
          </div>
      ...
      >
        <div className="bg-background rounded-md">
          <div
            className="bg-primary border-primary flex items-center justify-between gap-2 rounded-md border p-3 py-2"
            style={{
      ...
      <div className="flex gap-2">
        {[-10, -5, 0, 5, 10].map((percent) => (
          <div
            ...
            onClick={() => {
              setValue(lastValue * (1 + percent * 0.01));
            }}
          >
            {percent}%
          </div>
...
export function StockDrawer({
  ...
  const { data, isSuccess } = useQuery({
    ...
    queryFn: async () => {
      const res = apiClient.get<StockQuotaResponse>(`/api/stock/${symbol}`);
      return res;
    },
    enabled: !!symbol,
  });
